...
def generateLetter(
    letter, offset, noise, blur, rotation, font, textColour, backgroundColour
):
...
def drawText(text, inputImg, font, textColour):
    ...
    # Draw the text
    draw.text(
        (inputImg.shape[1] * 0.4, inputImg.shape[0] * 0.4),
        text_to_show,
        font=font,
        fill=(textColour),
    )
...
def Generate(toGen):
    ...
    for i in range(toGen):
        ...
        if i % 107 == 0:
            ...
            print("\033[A                             \033[A")
            print(
                f"{i}/{toGen}   [{'#'*percentDone}{'-'*(10-percentDone)}]    {round(i/toGen*100)}%    eta: {round(eta, 2)}s"
            )
            # cv2.imwrite(f"trainSetImg/{i}.png", im)
    print("\033[A\033[A")
    print(
        f"{toGen}/{toGen}   [{'#'*percentDone}{'-'*(10-percentDone)}]    {round(i/toGen*100)}%    Finished in: {time.time()-start_time}s"
    )
